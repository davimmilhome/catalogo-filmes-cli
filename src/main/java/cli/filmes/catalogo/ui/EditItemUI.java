...
public abstract class EditItemUI<T> extends TemplateUI {
    protected final T item;
    protected final EditItemCallback<T> editItemCallback;

    public EditItemUI(String title, T item, EditItemCallback<T> editItemCallback) {
        super(title);
        this.item = item;
    ...
    public boolean drawMenu() {
        ...
        if (option == 0) {
            ...
            boolean confirm = ConsoleUIHelper.askConfirm("Confirma a exclusão do item " + item.toString() + "?", "Sim", "Não");
            if (confirm) {
                keepShowing = false;
